    ...
    <template slot="items" slot-scope="props">
      <tr @click="toPlayerPage(props.item.attributes.id)">
        <td class="text-xs-center">
          {{ props.item.attributes.team_position }}</td>
        ...
        <td class="text-xs-center" style="font-size:16px;">{{ props.item.attributes.team_wins_total }}</td>
        <td class="text-xs-center">
          <v-icon color="#999">
            mdi-chevron-right
          </v-icon>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      teamPlayersList: state => state.leaderboards.team.teamPlayersList
    }),
  },
