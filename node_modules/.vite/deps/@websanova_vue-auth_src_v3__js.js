...
function setCookie(key, value, params) {
  ...
  for (i in params) {
    if (params[i] === false || params[i] === void 0) {
      continue;
...
function get(key) {
  var i, ii, cookie = document.cookie;
...
function set2(key, value, expires) {
  if (expires) {
...
function processToken(action, key, token, expires) {
  var i = 0, ts = this.options.stores, ii = ts.length, args = [getTokenKey.call(this, key)];
  if (action === "set") {
    args.push(token);
  ...
  for (; i < ii; i++) {
    ...
    if (ts[i] === "storage" && isLocalStorage() && isSessionStorage()) {
      return storage_exports[action].apply(this, args);
    }
    if (ts[i] === "cookie" && isCookieStorage()) {
      return cookie_exports[action].apply(this, args);
    }
  }
}
function get3(key) {
...
function set3(key, token, expires) {
...
function _getRemember() {
  return get3.call(__auth, __auth.options.rememberKey);
}
...
function _setStaySignedIn(staySignedIn) {
  if (staySignedIn === true) {
    set3.call(__auth, __auth.options.staySignedInKey, "true", false);
  } else {
    remove3.call(__auth, __auth.options.staySignedInKey);
  }
}
function _setRemember(val) {
  if (val) {
    set3.call(__auth, __auth.options.rememberKey, val, false);
    __auth.$vm.state.remember = val;
  } else {
    remove3.call(__auth, __auth.options.rememberKey);
...
function _parseRequestIntercept(req) {
  ...
  token = get3.call(__auth, tokenName);
  if (token) {
...
function _parseResponseIntercept(res, req) {
  ...
  if (token) {
    set3.call(__auth, null, token, get3.call(__auth, __auth.options.staySignedInKey) ? false : true);
  }
}
...
function _processAuthenticated(cb) {
  if (__auth.$vm.state.authenticated === null && get3.call(__auth)) {
...
function _processTransitionEach(transition, routeAuth, cb) {
  var authRedirect = (routeAuth || "").redirect || __auth.options.authRedirect, forbiddenRedirect = (routeAuth || "").forbiddenRedirect || (routeAuth || "").redirect || __auth.options.forbiddenRedirect, notFoundRedirect = (routeAuth || "").notFoundRedirect || (routeAuth || "").redirect || __auth.options.notFoundRedirect, rolesKey = (routeAuth || "").rolesKey || __auth.options.rolesKey;
  routeAuth = toArray((routeAuth || "").roles !== void 0 ? routeAuth.roles : routeAuth);
...
function _processLogout(redirect) {
  ...
  remove3.call(__auth, __auth.options.tokenImpersonateKey);
  remove3.call(__auth, __auth.options.tokenDefaultKey);
  remove3.call(__auth, __auth.options.staySignedInKey);
  __auth.$vm.state.loaded = true;
...
function _processImpersonate(defaultToken, redirect) {
  set3.call(__auth, __auth.options.tokenImpersonateKey, __auth.token(), get3.call(__auth, __auth.options.staySignedInKey) ? false : true);
  set3.call(__auth, __auth.options.tokenDefaultKey, defaultToken, get3.call(__auth, __auth.options.staySignedInKey) ? false : true);
...
function _initRefreshInterval() {
  if (__auth.options.refreshData.enabled && __auth.options.refreshData.interval > 0) {
    ...
    }, __auth.options.refreshData.interval * 1e3 * 60);
  }
}
...
Auth.prototype.impersonating = function() {
  var impersonating = get3.call(__auth, __auth.options.tokenImpersonateKey) ? true : false;
  if (__auth.$vm.state.impersonating === void 0) {
...
Auth.prototype.token = function(name, token, expires) {
  if (token !== void 0) {
    if (token === null) {
      remove3.call(__auth, name);
    } else {
      expires = expires === true || expires === false ? expires : get3.call(__auth, __auth.options.staySignedInKey) ? false : true;
      set3.call(__auth, name, token, expires);
    }
  }
  return get3.call(__auth, name);
};
...
Auth.prototype.login = function(data) {
  ...
  return new Promise(function(resolve, reject) {
    __auth.drivers.http.http.call(__auth, data).then(function(res) {
      if (data.fetchUser || data.fetchUser === void 0 && __auth.options.fetchData.enabled) {
        __auth.fetch({
...
Auth.prototype.remember = function(val) {
  ...
  if (__auth.$vm.state.remember === void 0) {
...
Auth.prototype.oauth2 = function(type, data) {
  ...
  Object.keys(data.params).forEach((key2) => {
    params.push(key2 + "=" + encodeURIComponent(data.params[key2]));
  });
  window.open(
...
// node_modules/@websanova/vue-auth/src/v3.js
var authKey = "auth";
function Vue(obj) {
  var data = obj.data();
  this.state = reactive(data.state);
}
auth_default.prototype.install = function(app, key) {
  app.provide(key || authKey, this);
  app.config.globalProperties.$auth = this;
};
function createAuth(options) {
  return new auth_default(Vue, options);
}
...
//# sourceMappingURL=@websanova_vue-auth_src_v3__js.js.map
